...
def test_analyze_content_item_returns_phoneme_metadata_for_cat() -> None:
    response = client.post(
        ...
        json={"prompt_id": "M2-001", "expected_text": "cat", "activity_type": "read_word", "module_key": "module_2"},
    )
...
def test_analyze_content_item_handles_missing_word_safely() -> None:
    response = client.post(
        "/analyze-content-item",
        json={"prompt_id": "M2-999", "expected_text": "zzzznotaword", "activity_type": "read_word", "module_key": "module_2"},
    )
    assert response.status_code == 200
